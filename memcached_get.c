                                           ...
                                           unsigned int number_of_keys)
{
  ...
  protocol_binary_request_getk getk_request= {0};
  memcached_return vk;
  protocol_binary_request_noop noop_request= {0};
  ...
  */
  for (x= 0; x < number_of_keys; x++) 
  {
    ...
    getk_request.message.header.request.magic= PROTOCOL_BINARY_REQ;
    if (number_of_keys == 1)
      getk_request.message.header.request.opcode= PROTOCOL_BINARY_CMD_GETK;
    else
      getk_request.message.header.request.opcode= PROTOCOL_BINARY_CMD_GETKQ;
    ...
    getk_request.message.header.request.keylen= htons((uint16_t)key_length[x]);
    getk_request.message.header.request.datatype= PROTOCOL_BINARY_RAW_BYTES;
    getk_request.message.header.request.bodylen= htonl(key_length[x]);
    
    if ((memcached_io_write(&ptr->hosts[server_key], getk_request.bytes,
                            sizeof(getk_request.bytes), 0) == -1) ||
  ...
  if (number_of_keys > 1) 
  {
     ...
     */    
    noop_request.message.header.request.magic= PROTOCOL_BINARY_REQ;
    noop_request.message.header.request.opcode= PROTOCOL_BINARY_CMD_NOOP;
    noop_request.message.header.request.datatype= PROTOCOL_BINARY_RAW_BYTES;
    ...
    for (x= 0; x < ptr->number_of_hosts; x++)
      if (memcached_server_response_count(&ptr->hosts[x])) 
      {
        ...
        if (memcached_io_write(&ptr->hosts[x], noop_request.bytes, 
			       sizeof(noop_request.bytes), 1) == -1) 
        {
          memcached_server_response_reset(&ptr->hosts[x]);
          memcached_io_reset(&ptr->hosts[x]);
          rc= MEMCACHED_SOME_ERRORS;
